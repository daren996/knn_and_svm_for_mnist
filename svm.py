...
class SimpleSvm:
    ...
    def __init__(self, C=5.0, tolerance=0.05, max_passes=20, kernel='linear'):
        self.C = C
...
def main(cluster_std=2.5):
    ...
    labelsArr = []
    data_ = np.array(data[0:500])
    labels_ = np.array(labels[0:500])
    for i in range(num_kind_labels):
    ...
    with open("svm_model_20", 'wb') as outf:
